...
final class DeletePostViewModel: ViewModelType {
...
extension DeletePostViewModel {
    private func deletePostAPI(accessToken: String, contentId: Int) async throws -> BaseResponse<EmptyResponse>? {
        let accessToken = accessToken
        do {
            let result: BaseResponse<EmptyResponse>? = try
            await self.networkProvider.donNetwork(type: .delete, baseURL: Config.baseURL + "/content/\(contentId)", accessToken: accessToken, body: EmptyBody(), pathVariables: ["":""])
